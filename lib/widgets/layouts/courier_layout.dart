...
class _CourierLayoutState extends State<CourierLayout> {
  ...
  Widget build(BuildContext context) {
    final List<Widget> screens = [
      ...
      _CourierRouteScreen(
        tasks: _tasks.where((t) => t.status == CourierTaskStatus.inProgress).toList(),
        isLoading: _isLoading,
        onRefresh: _fetchTasks,
      ),
      const ProfileScreen(),
...
class CourierTasksScreen extends StatelessWidget {
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        ...
        body: isLoading 
          ...
          : TabBarView(
              children: [
                _buildTaskList(tasks.where((t) => t.status == CourierTaskStatus.assigned).toList()),
                _buildTaskList(tasks.where((t) => t.status == CourierTaskStatus.inProgress).toList()),
              ],
            ),
      ),
    );
  ...
  Widget _buildTaskList(List<CourierTask> taskList) {
    if (taskList.isEmpty) {
      return const Center(
        child: Text('НЕТ ЗАДАЧ В ЭТОМ РАЗДЕЛЕ', style: TextStyle(color: Color(0xFF171717), fontWeight: FontWeight.bold)),
      );
    }
  ...
  Future<void> _handleAction() async {
    ...
    try {
      if (widget.task.status == CourierTaskStatus.assigned) {
        await _repository.updateCourierTaskStatus(widget.task.id, status: 'in_progress');
        ...
        child: Column(
          ...
          children: [
            Row(
              ...
              children: [
                Container(
                  ...
                  color: isColorLab ? const Color(0xFFF01D2C) : const Color(0xFF171717),
                  child: Text(
            ...
            Text(
              ...
              style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 12, color: Color(0xFFF01D2C)),
            ),
            const SizedBox(height: 8),
            Row(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Icon(CupertinoIcons.location_solid, size: 20, color: Color(0xFFF01D2C)),
                const SizedBox(width: 10),
                Expanded(
                  child: Text(
                    widget.task.address.toUpperCase(),
                    style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w800, color: Color(0xFF171717), height: 1.3),
                  ),
                ),
              ],
            ),
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                style: ElevatedButton.styleFrom(
                  backgroundColor: widget.task.status == CourierTaskStatus.assigned ? const Color(0xFFF01D2C) : const Color(0xFF171717),
                ...
                child: _processing 
                  ...
                  : Text(
                      widget.task.status == CourierTaskStatus.assigned ? 'ПРИНЯТЬ В РАБОТУ' : 'ЗАВЕРШИТЬ (ФОТО)',
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: isLoading 
        ? const Center(child: CircularProgressIndicator(color: Color(0xFFF01D2C)))
        : tasks.isEmpty 
          ? const Center(child: Text('НЕТ АКТИВНЫХ ЗАДАЧ В ПУТИ', style: TextStyle(fontWeight: FontWeight.bold)))
          : ListView.builder(
              padding: const EdgeInsets.all(16),
              ...
              itemBuilder: (context, index) {
                ...
                return Container(
                  ...
                  decoration: const BoxDecoration(
                    color: Colors.white,
                    border: Border(left: BorderSide(color: Color(0xFFF01D2C), width: 5)),
                  ),
                  child: ListTile(
                    contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
                    title: Text(task.address.toUpperCase(), style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 13)),
                    subtitle: Text('${task.timeSlot} · ${task.clientName}', style: const TextStyle(fontSize: 11, fontWeight: FontWeight.bold)),
